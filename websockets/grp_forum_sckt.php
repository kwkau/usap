...
class grp_forum_sckt extends wsserver{

    public function __construct($addr, $port)
    {
        parent::__construct($addr,$port);
        $this->channel = "groupForumSocket";
    }
    ...
    protected function onopen(Socket $sckt, $data)
    {
        ...
        if(is_array($packet)){
            ...
            $this->add_user($sckt)?$this->post($sckt,"grp_forum_load",$this->load_forums($pckt_data["group_id"])):$this->post($sckt,"grp_forum_load","invalid");
        ...
        if(is_array($packet)) {
            ...
            if($payload["type"] == "forum"){
                ...
                //obtain target
                     ...
                     *------------------------------------------------------------------------*/
                    $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_forum_creation",$forum_data);
                 ...
                 *--------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_forum_comment_creation",$forum_comment_data);
                ...
                $this->post($sckt,"grp_forum_comment_load",$comments);
                 ...
                 *------------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_forum_comment_smiley_creation",$forum_comment_smiley_data);
                 ...
                 * broadcast the forum comment like to all users that are connected to the server
                 *--------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_forum_comment_smiley_del",$forum_comment_smiley_data);
    ...
    private function load_forums($id)
    {
        ...
        return $forum_obj->get_forums($id,"group");
    }
    ...
    protected function insert_forum(Socket $sckt, array $forum_data)
    {
        ...
        $forum->group_id = $forum_data["group_id"];
