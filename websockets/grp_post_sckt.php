...
class grp_post_sckt extends wsserver{

    public function __construct($addr, $port)
    {
        parent::__construct($addr,$port);
        $this->channel = "groupPostSocket";
    }
    ...
    protected function onopen(Socket $sckt, $data)
    {
        ...
        if(is_array($packet)){
            ...
            $group->fetch_group($pckt_data["group_name"]);
            $sckt->user->group = $group;
    ...
    protected function onmessage(Socket $sckt, $data)
    {
        ...
        if(is_array($packet)) {
            ...
            if($payload["type"] == "post"){
                 ...
                 *----------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_post_creation",$post_data);
                 ...
                 *-----------------------------------------------------------------*/
                $this->post($sckt,"grp_post_load",$this->load_posts($payload["group_id"]));
                 ...
                 *------------------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_post_smiley_creation",$post_smiley_data);
                 ...
                 *--------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_post_smiley_del",$post_smiley_data);
                 ...
                 *--------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_post_comment_creation",$post_comment_data);
                ...
                $this->post($sckt,"grp_post_comment_load",$comments);
                 ...
                 *-----------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_post_comment_smiley_creation",$post_comment_smiley_data);
                 ...
                 * broadcast the post comment like to all users that are connected to the server
                 *-----------------------------------------------------------------------------*/
                $this->broadcast($sckt,$this->group_mem_sckts($sckt,$sckt->user->group->name),"grp_post_comment_smiley_del",$post_comment_smiley_data);
    ...
    private function load_posts($group_id)
    {

        $post = new post_mdl();
        return $post->get_posts($group_id,"group");
    }
